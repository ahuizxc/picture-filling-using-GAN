...
def preprocess_image(images, batch_size, image_size_h,image_size_w, hidden_size_h,hidden_size_w,
                     image_dim, random_block=True, target_image=None):# single random blocks
  masks = np.ones((batch_size, image_size_h, image_size_w,
                   image_dim), dtype=np.float32)
  hiding_images = np.zeros(
      (batch_size, hidden_size_h, hidden_size_w, image_dim), dtype=np.float32)
  masks_idx = None
  if random_block:
    masks_idx = np.random.randint(0, hidden_size_w, (batch_size, 2), np.int)
    for idx in range(batch_size):
      idx_start1 = int(masks_idx[idx, 0])
      idx_end1 = int(masks_idx[idx, 0] + (hidden_size_w))
      idx_start2 = int(masks_idx[idx, 1])
      idx_end2 = int(masks_idx[idx, 1] + (hidden_size_h))
    ...
    masks_idx = [0,0]    
    masks[:, masks_idx[0]:masks_idx[0] + hidden_size_h,
          masks_idx[1]:masks_idx[1] + hidden_size_w, :] = 0
    hiding_images = np.copy(images[:, masks_idx[0]:masks_idx[0] + hidden_size_h,
                                   masks_idx[1]:masks_idx[1] + hidden_size_w, :])
  ...
  if target_image is not None:
    target_image = np.reshape(
        target_image, (batch_size, hidden_size_h, hidden_size_w, image_dim))
    for idx in range(batch_size):
      idx_start1 = int(masks_idx[idx, 0])
